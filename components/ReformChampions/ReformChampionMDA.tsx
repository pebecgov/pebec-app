...
import { FaChevronDown, FaChevronUp, FaAngleDoubleLeft, FaAngleDoubleRight, FaFileAlt, FaEnvelopeOpenText, FaEnvelope, FaEnvelopeOpen, FaCalendar } from "react-icons/fa";
import { LayoutDashboard, LogOut, User } from "lucide-react";
export default function DeputiesSidebar({
  isOpen,
  setIsOpen
}) {
  ...
  const menuSections = [{
    ...
    path: "/reform_champion"
  }, {
    name: "Send letter",
    icon: <FaEnvelope className="w-5 h-5" />,
    path: "/reform_champion/send-letters"
  }, {
    name: "Received letters",
    icon: <FaEnvelopeOpen className="w-5 h-5" />,
    path: "/reform_champion/received-letters"
  }, {
    name: "Meetings",
    icon: <FaCalendar className="w-5 h-5" />,
    path: "/reform_champion/meetings"
  }, {
    name: "Profile",
    icon: <User className="w-5 h-5" />,
    path: "/reform_champion/profile"
  }];
  return <>
      {}
      <aside className={`bg-white shadow-lg h-screen fixed transition-all duration-300 z-50 border-r border-gray-200 flex flex-col 
        ...
        <nav className="mt-6 flex-grow overflow-y-hidden hover:overflow-y-auto">
          ...
          <div title={!isOpen ? section.name : undefined} className={`flex items-center p-4 rounded-lg cursor-pointer hover:bg-gray-100 transition-all duration-300 ${pathname === section.path ? "bg-green-500 text-white" : ""}`}>
            {section.icon}
