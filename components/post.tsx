...
export default function Post({ slug }: { slug: string }) {
  ...
  const convexUser = useQuery(
    ...
    safeClerkUserId ? { clerkUserId: safeClerkUserId } : "skip"
  );
  ...
  if (!post) {
    return (
      <section className="pb-24 pt-20 sm:pt-25">
  ...
  return (
    <section className="pb-12 pt-20 bg-white dark:bg-black min-h-screen">
      <div className="container max-w-3xl mx-auto px-4 sm:px-0">
        {/* Back Button */}
        <button
          onClick={() => router.push("/portal")}
          className="mb-8 flex items-center text-sm text-gray-500 hover:text-gray-800 transition-all"
        >
        ...
        {/* Cover Image */}
        {post.coverImageUrl && (
          <div className="w-full mb-8 rounded-md overflow-hidden border">
            <img
        ...
        <div className="flex justify-between items-center border-y py-4 mb-6 text-gray-600 dark:text-gray-300">
          <button
            onClick={() => likePost({ slug: post.slug })}
