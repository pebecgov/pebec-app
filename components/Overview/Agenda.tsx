...
export default function ReformSlider() {
  ...
  return <section className="relative w-full max-w-6xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <h2 className="text-center text-4xl font-extrabold text-green-700 mb-10">The Reform Agenda</h2>
      
      {}
      <div className="md:hidden flex justify-center mb-6">
        <button className="border p-2 rounded-lg bg-gray-200 text-gray-700 flex items-center" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          Select Reform
        </button>
        {mobileMenuOpen && <div className="absolute top-20 bg-white shadow-lg rounded-lg p-4 z-50 w-60">
            {reforms.map((reform, index) => <button key={index} onClick={() => {
      ...
      <div className="relative flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="hidden md:flex flex-col items-center gap-2 w-40 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-2 w-full h-[250px] overflow-hidden">
            {reforms.map((reform, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`border p-3 rounded-md w-full flex items-center cursor-pointer transition-all ${activeIndex === index ? "border-blue-500 bg-blue-200 scale-105 shadow-lg" : "border-gray-200 bg-white"}`}>
                {reform.icon}
