...
export default function UserDLIReportModal({
  open,
  onClose
}: {
  open: boolean;
  ...
  const allProgressByState = useQuery(api.dli.getStateDLIsReformChampion, {
    state: userState
  });
  const userDLIs = useMemo(() => {
    ...
    return allProgressByState.filter(d => (isOwner(d) || isGovernor && sameState(d)) && (d.status === "in_progress" || d.status === "completed"));
  ...
  const handleGenerate = async () => {
    ...
    cursorY += 80;
    const records = selectedDLIs.includes("ALL") ? userDLIs : userDLIs.filter(d => selectedDLIs.includes(d.dliTemplateId));
      ...
      <DialogContent className="max-w-xl">
        ...
        {step === 1 && <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-64 overflow-y-auto">
              <Card onClick={() => setSelectedDLIs(["ALL"])} className={`p-4 cursor-pointer border ${selectedDLIs.includes("ALL") ? "border-blue-500" : "hover:border-gray-400"}`}>
                All My DLIs
              </Card>
              {userDLIs.map(p => <Card key={p.dliTemplateId} onClick={() => setSelectedDLIs(prev => prev.includes(p.dliTemplateId) ? prev.filter(id => id !== p.dliTemplateId) : [...prev.filter(id => id !== "ALL"), p.dliTemplateId])} className={`p-4 cursor-pointer border ${selectedDLIs.includes(p.dliTemplateId) ? "border-blue-500" : "hover:border-gray-400"}`}>
