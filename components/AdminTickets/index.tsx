...
export default function AdminTicketsPage() {
  ...
  const exportToExcel = () => {
    const data = filteredTickets.filter(ticket => selectedTickets.includes(ticket._id));
    ...
    <div className="w-full sm:w-44 h-12">
      <Select onValueChange={value => setStatusFilter(value as any)}>
        ...
        <SelectContent className="bg-zinc-800 text-white">
          {TABS.map(tab => <SelectItem key={tab.value} value={tab.value}>
              {tab.label}
            </SelectItem>)}
        </SelectContent>
      </Select>
  ...
  <div className="flex flex-col w-full sm:w-40">
    ...
    <div className="flex flex-wrap gap-2 mt-2 sm:mt-0">
      ...
      <Button className="bg-green-600 text-white hover:bg-green-700 h-12" onClick={() => setShowTicketReportModal(true)}>
        📄 Generate Report
      </Button>

      <Button className="bg-blue-600 text-white hover:bg-blue-700 h-12" onClick={() => setShowMonthlyReportModal(true)}>
        📅 Monthly Report
      </Button>
    </div>
  </div>
  ...
  <DialogContent>
    ...
    <Textarea placeholder="Enter your note..." value={resolutionNote} onChange={e => setResolutionNote(e.target.value)} />
    <DialogFooter>
    ...
    <DialogFooter className="mt-4">
      ...
      <Button onClick={() => {
            if (pendingStatusChange) {
              handleStatusChange(pendingStatusChange.ticketId, pendingStatusChange.newStatus);
              setShowConfirmDialog(false);
              setPendingStatusChange(null);
            }
          }}>
