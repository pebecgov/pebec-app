...
export default function FileUploader({ setFileId }: FileUploaderProps) {
  ...
  const saveUploadedFile = useMutation(api.tickets.saveUploadedFile); // 👈 new mutation for saving name
  ...
  async function handleUpload() {
    ...
    try {
      ...
      // Step 3: Save filename mapped to storageId
      await saveUploadedFile({ storageId, fileName: file.name });

      setFileId(storageId, file.name); // still useful for parent component
