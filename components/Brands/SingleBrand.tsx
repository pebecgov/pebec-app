...
const SingleBrand: React.FC<Props> = ({
  brand
}) => {
  return <div className="relative group flex justify-center items-center">
      <Link href={brand.href}>
        <div className="flex items-center justify-center w-16 h-12 md:w-20 md:h-14 transition-transform duration-200 hover:scale-105">
          <Image src={brand.image} alt={brand.name} fill className="object-contain dark:hidden" />
          <Image src={brand.imageLight} alt={brand.name} fill className="object-contain hidden dark:block" />
        </div>
      </Link> 
