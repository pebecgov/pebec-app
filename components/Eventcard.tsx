...
export default function EventItem({
  ...
  const shortDescription = descriptionWords.length > 30 ? descriptionWords.slice(0, 30).join(" ") + "..." : event.description;
  ...
  return <li className="max-w-lg mx-auto bg-white rounded-lg shadow-md overflow-hidden sm:max-w-2xl m-3 relative transition-transform hover:scale-[1.02]">
      <Link href={`/events/${event._id}`} className="block">
        <div className="flex flex-col sm:flex-row">
          ...
          {}
          {coverImageUrl && <div className="w-full sm:w-48 h-48 relative">
              <Image alt="Event Cover" src={coverImageUrl} layout="fill" objectFit="cover" className="w-full h-full object-cover" />
            </div>}
