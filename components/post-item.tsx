...
export default function PostItem({ post, showComments = false, isNew = false }: Props) {
  const { userId } = useAuth();
  const router = useRouter();
  const deletePost = useMutation(api.posts.deletePost);

  const comments = useQuery(
    api.posts.getCommentsByPost,
    post ? { postId: post._id as Id<"posts"> } : "skip"
  );

  const currentUser = useQuery(api.users.getCurrentUsers); // assumes you have this query

  const isAuthor =
  currentUser?._id === post.authorId || currentUser?.role === "admin";
    ...
    <li className="relative mb-6">
      <Link href={`/posts/${post.slug}`} className="block group">
        <div className="bg-white dark:bg-gray-900 shadow-md rounded-lg p-5 transition hover:shadow-lg border border-gray-100 dark:border-gray-800">
          {/* NEW Badge */}
          {isNew && (
            <span className="absolute top-3 right-3 bg-green-600 text-white text-xs font-semibold px-2 py-1 rounded shadow">
              NEW
            </span>
          )}
                ...
                <Dialog>
                  ...
                  <DialogContent>
                    ...
                    <DialogFooter className="mt-4">
                      ...
                      <Button
                        variant="destructive"
                        onClick={async () => await deletePost({ slug: post.slug })}
                      >
                        Confirm
                      </Button>
                    </DialogFooter>
