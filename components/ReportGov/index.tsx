...
export default function ReportGovPage() {
  ...
  const handleSubmitComplaintClick = async () => {
    if (isSignedIn) {
      ...
      if (primaryEmail && currentSignedInEmail && primaryEmail !== currentSignedInEmail) {
        // ❗ Secondary email detected, force logout and redirect
        await signOut(); // You need to import `useClerk` to access signOut
  ...
  disabled={!!user && user.publicMetadata?.role !== "user"} // disables ONLY non-user roles
...
>
            ...
            {/* <div className="flex items-center gap-4 cursor-pointer" onClick={() => setShowVideo(true)}>
              <div className="border-2 border-green-900 w-12 h-12 rounded-full flex items-center justify-center">
                <FaPlay className="text-green-900" />
              </div>
              <div className="text-left">
                <p className="text-sm font-semibold text-green-900">Watch Demo</p>
                <p className="text-sm text-gray-800">See how it works</p>
              </div>
            </div> */}
          </div>
        </div>
