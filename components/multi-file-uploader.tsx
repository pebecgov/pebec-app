...
interface Props {
  setFileIds: (ids: { storageId: string; fileName: string }[]) => void; // 🆙 Now includes filenames
}
...
export default function MultiFileUploader({ setFileIds }: Props) {
  ...
  const saveUploadedFile = useMutation(api.tickets.saveUploadedFile); // 🆕 Save filename after upload
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // Save storageId + original fileName to DB
      ...
      {uploadedFiles.length > 0 && (
        <ul className="text-xs text-green-600 mt-2">
          {uploadedFiles.map((file, idx) => (
            <li key={idx}>✔ {file.fileName}</li> // 🆙 Now showing real file names!
          ))}
        </ul>
      )}
    </div>
