...
export default function ReportTemplatesList() {
  ...
  const reportTemplates = useQuery(api.internal_reports.getReportTemplates, selectedRole === "all" ? "skip" : {
    role: selectedRole
  }) || [];
  return <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportTemplates.length > 0 ? reportTemplates.map(template => <div key={template._id} className="p-4 border rounded-lg shadow-md bg-gray-100">
          ...
          setIsDialogOpen(true);
            ...
            </div>) : <p className="text-gray-500">No reports available for this role.</p>}
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          {selectedTemplate && <FillReportForm template={selectedTemplate} onClose={() => setIsDialogOpen(false)} />}
        </DialogContent>
      </Dialog>
    </div>;
}