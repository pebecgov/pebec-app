...
export function PostMediaModal({
  ...
  return <Dialog open={open} onOpenChange={isOpen => {
    ...
    <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex flex-col gap-4 overflow-y-auto pr-2" style={{
        maxHeight: '70vh'
      }}>

        <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title" />
        <Textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Write your description here..." />

        <select className="border rounded px-3 py-2" value={categoryId} onChange={e => setCategoryId(e.target.value as Id<"mediaCategories">)}>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {previewUrls.map((src, idx) => <Image key={idx} src={src} alt={`preview-${idx}`} width={100} height={100} className="rounded border" />)}
        </div>

        {videoLinks.map((link, idx) => <div key={idx} className="flex items-center gap-2 mt-2">
    <Input value={link} onChange={e => {
            const updated = [...videoLinks];
            updated[idx] = e.target.value;
            setVideoLinks(updated);
          }} placeholder="Video URL (YouTube, Vimeo)" className="flex-1" />
    <Button size="sm" variant="destructive" onClick={() => handleRemoveVideo(idx)} className="h-10">
      Delete
    </Button>
  </div>)}


        <Button size="sm" variant="ghost" onClick={handleAddVideo} className="mt-1">
          + Add Video Link
        </Button>
