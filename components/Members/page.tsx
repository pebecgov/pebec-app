...
const PEBECMembers = () => {
  ...
  return <div className="font-[sans-serif] pb-16 ">
      {}
      <section className="relative w-full h-[300px] md:h-[320px] bg-cover bg-center bg-no-repeat flex items-center mt-[-60px] pt-[80px]" style={{
      backgroundImage: "url('/images/members_banner.png')"
    }}>
  ...
  <div className="relative z-10 w-full px-6 md:px-12">
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
      ...
      <div className="w-full max-w-md">
        <input type="text" className="w-full p-4 text-sm md:text-base rounded-full shadow-md text-gray-900 border border-gray-300 focus:ring-2 focus:ring-green-500 focus:outline-none" placeholder="Search members..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
      </div>
    </div>
  </div>
    </section>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-8">
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-6 text-center">
          {filteredMembers.map(member => <div key={member.id} className="bg-white rounded-lg p-4 shadow-md hover:scale-105 transition-all duration-500 border border-gray-200 relative">
              {}
              ...
              <div className="mt-4">
                ...
                <div className="flex justify-center space-x-3 mt-3">
                  {member.socials.facebook && <a href={member.socials.facebook} target="_blank" className="text-gray-500 hover:text-blue-600">
                      <i className="fab fa-facebook-f"></i>
                    </a>}
                  {member.socials.twitter && <a href={member.socials.twitter} target="_blank" className="text-gray-500 hover:text-blue-400">
                      <i className="fab fa-twitter"></i>
                    </a>}
                  {member.socials.linkedin && <a href={member.socials.linkedin} target="_blank" className="text-gray-500 hover:text-blue-800">
