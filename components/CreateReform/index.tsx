...
export default function CreateReformPage() {
  ...
  const handleCreateReform = async () => {
    ...
    toast({
      title: "Success!",
      description: "Reform created successfully!"
    });
    setForm({
      title: "",
      description: "",
  ...
  return <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h1 className="text-2xl font-bold text-center">Create New Reform</h1>

      {}
      <ImageUploader setImageId={(storageId: Id<"_storage">) => setForm(prev => ({
      ...prev,
      imageId: storageId
    }))} />
      ...
      category: e.target.value
        ...
        {categories.map((category, idx) => <option key={idx} value={category}>
            {category}
          </option>)}
      </select>
      ...
      <Input type="date" value={form.implementedDate} onChange={e => setForm({
      ...form,
      implementedDate: e.target.value
    })} required />
      ...
      <div className="mt-6">
        ...
        {form.impact.map((impact, index) => <div key={index} className="flex items-center gap-2 mt-2">
            <Input type="text" placeholder="Enter impact" value={impact} onChange={e => {
          ...
          setForm({
            ...
            {index > 0 && <Button type="button" variant="destructive" onClick={() => handleRemoveItem("impact", index)}>
      ...
      <div className="mt-6">
        ...
        {form.outcome.map((outcome, index) => <div key={index} className="flex items-center gap-2 mt-2">
            <Input type="text" placeholder="Enter outcome" value={outcome} onChange={e => {
          ...
          setForm({
            ...
            {index > 0 && <Button type="button" variant="destructive" onClick={() => handleRemoveItem("outcome", index)}>
                <Trash className="w-5 h-5" />
              </Button>}
          </div>)}
