...
export default function FullNotifications() {
  ...
  return <div className="max-w-4xl mx-auto px-4 py-8">
    ...
    {notifications.length === 0 ? <p className="text-gray-500">You have no notifications.</p> : <ul className="space-y-4">
      {notifications.map(notification => <li key={notification._id as string} className="border rounded-md p-4 shadow-sm bg-white flex flex-col sm:flex-row justify-between sm:items-center">
        <div>
          ...
          {notification.ticketId && <Link href={`/${getRolePath()}/tickets/${notification.ticketId}`} className="text-blue-600 text-sm hover:underline">
            Go to Ticket →
          </Link>}
          ...
          {notification.taskId && <Link href={`/${getRolePath()}/tasks`} className="text-blue-600 text-sm hover:underline">
            Go to Task →
          </Link>}
        </div>

        <div className="flex gap-2 mt-2 sm:mt-0">
          {!notification.isRead && <Button size="sm" onClick={() => markAsRead({
