...
export default function UserOpenTickets() {
  ...
  const mdaUsers = useQuery(api.users.getUsersWithRole, { role: "mda" }) || []; // ✅ Get MDA users
  ...
  const mdas = useQuery(api.users.getMDAs) || []; // ✅ Fetch all MDAs safely
  ...
  // ✅ Function to check if a ticket has new notifications
  ...
  // Function to return status badge styles & progress
  const getStatusDetails = (status: string) => {
   ...
   // ✅ Function to find assigned MDA Name
  // ✅ Function to get MDA Name Correctly
...
const handleCancelTicket = async () => {
  ...
  try {
    ...
    // ✅ Send Emails to User, Admins, and MDA
...
const handleReopenTicket = async (ticketId: Id<"tickets">) => {
  ...
  try {
    ...
    // ✅ Send Emails to User, Admins, and MDA
    await sendEmails(ticket, "reopen", mdaUsers);  // 👈 Fix here! Change "cancel" to "reopen"
...
async function sendEmails(ticket, action, mdaUsers) {
  ...
  // ✅ Find the MDA user assigned to this ticket
  ...
  <Button 
    ...
    disabled={ticket.status === "closed" || loading === ticket._id} // Disable if already canceled
  >
    {loading === ticket._id ? "Canceling..." : "Cancel"}
  </Button>
) : ticket.status === "closed" && !ticket.resolvedByAdmin ? ( 
  // ✅ Only allow reopen if the ticket was closed by the user, NOT if resolved by admin/MDA
  <Button onClick={() => handleReopenTicket(ticket._id)} disabled={loading === ticket._id}>
    {loading === ticket._id ? "Reopening..." : "Reopen"}
  </Button>
) : (
  // ✅ Hide reopen button if ticket was resolved by admin/MDA
