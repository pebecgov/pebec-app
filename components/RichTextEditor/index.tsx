...
import {
  Bold, Italic, Strikethrough, Link as LinkIcon, List,
  ListOrdered, PaintBucket
} from "lucide-react";
...
export default function RichTextEditor({ value, onChange }: { value: string, onChange: (val: string) => void }) {
  ...
  <DialogContent>
    ...
    />
    {/* Wrap button with DialogClose to auto-close on click */}
    <DialogClose asChild>
      <Button
        onClick={() => {
          if (linkUrl) {
      ...
      <EditorContent 
  editor={editor} 
  className="w-full min-h-[200px] p-4 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 prose prose-sm [&_a]:underline [&_a]:text-blue-600"
/>
