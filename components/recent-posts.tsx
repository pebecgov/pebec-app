      ...
      <CardContent>
        ...
        <ul className="flex flex-col gap-4">
          {posts.map(post => <li key={post._id}>
              <Link href={`/posts/${post.slug}`} className="flex items-center gap-4 hover:bg-muted/40 rounded-md px-4 py-2 transition">
                {}
                <div className="w-10 h-10 rounded-full overflow-hidden border bg-gray-100 flex-shrink-0">
                  {coverUrls[post._id] ? <Image src={coverUrls[post._id]} alt={post.title} width={40} height={40} className="object-cover w-full h-full" /> : <div className="flex items-center justify-center w-full h-full text-xs text-gray-400">
                      No Image
                    </div>}
                </div>
