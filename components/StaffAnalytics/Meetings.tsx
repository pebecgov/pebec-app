      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-gray-50 border rounded-2xl p-6 shadow-sm space-y-6">
            ...
            {thisWeeksMeetings.length > 0 ? thisWeeksMeetings.map(slot => <MeetingItem key={slot._id} slot={slot} />) : <p className="text-sm text-gray-400">
                No meetings scheduled for this week.
              </p>}
          </div>

          {isEndOfWeek && nextWeeksMeetings.length > 0 && <div className="bg-gray-50 border rounded-2xl p-4 shadow-sm space-y-4">
              <h3 className="text-md font-semibold text-gray-800">
                🔜 Upcoming Next Week
              </h3>
              {nextWeeksMeetings.slice(0, 2).map(slot => <div key={slot._id} className="border rounded-lg p-3 bg-white shadow-sm">
              ...
              <a href="/staff/meetings" className="block text-center text-sm font-semibold text-blue-600 hover:text-blue-800 transition">
                ➕ View All Meetings
              </a>
            </div>}
        </div>
