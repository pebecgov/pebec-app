...
export default function UploadReports({
  ...
  Supported formats:{" "}
  {reportType === "BFA Report" ? "XLS, XLSX, CSV" : "PDF, DOC, DOCX"}{" "}
  (Max {MAX_FILE_SIZE_MB}MB)
        ...
        {file && <div className="mt-4 bg-gray-100 p-3 rounded-md flex items-center justify-between gap-4 text-sm overflow-hidden">
    <div className="flex-1 overflow-hidden">
      <p className="truncate text-gray-800" title={file.name}>
        {file.name}
      </p>
    </div>
    <Button size="sm" className="shrink-0 bg-red-500 text-white" onClick={() => setFile(null)}>
      Remove
    </Button>
  </div>}
