...
export default function GenerateTopMdasReport() {
  ...
  const reportData = useQuery(api.tickets.getMDAReports, fromDate && toDate && submitted && mdaIds.length > 0 ? {
    fromDate,
    toDate,
    mdaIds
  } : "skip");
  ...
  const getTopMdas = () => {
    ...
    return Object.values(grouped).sort((a, b) => b.count - a.count).map((mda, i) => ({
      rank: i + 1,
      name: mda.mdaName,
      count: mda.count
    }));
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-2">
              <label>From</label>
              <input type="date" className="border rounded px-3 py-2" onChange={e => setFromDate(new Date(e.target.value).getTime())} />
            </div>
            <div className="flex flex-col gap-2">
              <label>To</label>
              <input type="date" className="border rounded px-3 py-2" onChange={e => setToDate(new Date(e.target.value).getTime())} />
            </div>
            <Button onClick={handleGenerate} disabled={loading || !fromDate || !toDate} className="w-full bg-blue-600 text-white">
              {loading ? "Generating..." : "Generate"}
            </Button>

            {reportData && reportData.length > 0 && <div className="flex flex-col gap-2 mt-4">
                <Button onClick={exportPDF} className="bg-blue-500 text-white">
                  <FileDown className="mr-2 w-4 h-4" /> Export PDF
                </Button>
                <Button onClick={exportExcel} className="bg-green-500 text-white">
