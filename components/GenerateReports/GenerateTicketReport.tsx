...
export default function GenerateTicketReport({ open, onClose }: Props) {
  ...
  const mdas = useQuery(api.tickets.getAllMdas); // or `api.users.getMDAs` if that's what you use
  ...
  const downloadPDF = () => {
    ...
    doc.text(filterHeader, 14, 35); // 👈 Add the filter header
  
    const tableData = filteredData.map((t) => {
      const row = [
        t.ticketNumber || "—",
        t.fullName || "—",
        t.businessName || "—", // ✅ Add Business Name
        t.title || "—",
      ];
    ...
    const pdfHead = [
      "Ticket #", "Name",   "Business Name", // ✅ Add Business Name
"Title",
  ...
  const downloadExcel = () => {
    const dataRows = filteredData.map((t) => {
      const base = {
        ...
        Name: t.fullName || "—",
        BusinessName: t.businessName || "—", // ✅ Add Business Name
        Title: t.title || "—",
        Status: t.status,
      ...
      if (showMdaColumn) {
        return {
          ...base,
          MDA: t.assignedMDAName || "Unassigned", // ✅ conditionally add MDA
        };
      }
    ...
    const columnHeaders = [
      "TicketNumber", "Name",   "BusinessName", // ✅ Add header
"Title",
      ...(showMdaColumn ? ["MDA"] : []),
...
{step === 2 && filterMode === "mda" && (
  ...
  onSelect={() => {
    setSelectedMDA("all");
    setOpenUserDropdown(false); // ✅ Close on select
  }}
>
  All MDAs
</CommandItem>
