...
export default function MDAReports() {
  ...
  const fetchedReportData = useQuery(api.tickets.getMDAReports, fromDate && toDate && selectedMDAIds.length > 0 ? {
    fromDate: new Date(fromDate).getTime(),
    toDate: new Date(toDate).getTime(),
    mdaIds: selectedMDAIds
  } : "skip");
  const userIds = fetchedReportData ? [...new Set(fetchedReportData.map(ticket => ticket.createdBy))] : [];
  const users = useQuery(api.users.getUsersByIds, userIds.length > 0 ? {
    userIds
  } : "skip");
  useEffect(() => {
  ...
  const handleMDASelection = (value: string) => {
    setSelectedMDAIds(prev => prev.includes(value as Id<"mdas">) ? prev.filter(id => id !== value as Id<"mdas">) : [...prev, value as Id<"mdas">]);
  };
  const handleExportPDF = () => {
    ...
    autoTable(doc, {
      startY: 20,
      head: [["No.", "Ticket Number", "Status", "Full Name", "Incident Date", "State", "Address", "Resolution Note", "Phone Number", "Email Address", "Business Name", "Assigned MDA"]],
      body: reportData.map(ticket => [ticket.number, ticket.ticketNumber, ticket.status, ticket.fullName, ticket.incidentDate, ticket.state, ticket.address, ticket.resolutionNote, ticket.phoneNumber, ticket.email, ticket.businessName, ticket.assignedMDAName])
    });
    doc.save("MDA_Ticket_Report.pdf");
  };
  const handleExportExcel = () => {
    const headers = ["No.", "Ticket Number", "Status", "Full Name", "Incident Date", "State", "Address", "Resolution Note", "Phone Number", "Email Address", "Business Name", "Assigned MDA"];
    const data = reportData.map(ticket => [ticket.number, ticket.ticketNumber, ticket.status, ticket.fullName, ticket.incidentDate, ticket.state, ticket.address, ticket.resolutionNote, ticket.phoneNumber, ticket.email, ticket.businessName, ticket.assignedMDAName]);
  ...
  return <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-8">
      ...
      <div className="flex flex-wrap gap-4 mb-6 p-6 bg-white border rounded-lg shadow-lg">
        ...
        <Select onValueChange={handleMDASelection}>
          ...
          <SelectContent>
            {mdas.map(mda => <SelectItem key={mda._id} value={mda._id}>
                {mda.name}
              </SelectItem>)}
          </SelectContent>
        </Select>
      ...
      {reportData.length > 0 && <>
          <Table>
            ...
            <TableBody>
              {reportData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map(ticket => <TableRow key={ticket.ticketNumber}>
