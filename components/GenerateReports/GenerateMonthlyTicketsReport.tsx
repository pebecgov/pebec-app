...
export default function GenerateMonthlyReport({
  ...
  const downloadPDF = () => {
    ...
    const tableData = filteredData.map(t => [t.ticketNumber || "—", t.fullName || "—", t.businessName || "—", t.title || "—", t.status, format(new Date(t._creationTime), "PPP"), t.state || "—", t.address || "—", t.email || "—", t.phoneNumber || "—", t.description?.replace(/<[^>]+>/g, "") || "", t.resolutionNote || ""]);
    autoTable(doc, {
      startY: 32,
      head: [["Ticket #", "Name", "Business", "Title", "Status", "Submitted on", "State", "Address", "Email", "Phone Number", "Description", "Resolution note"]],
      body: tableData,
      styles: {
        fontSize: 6
      },
  ...
  return <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl">
        ...
        <DialogFooter className="mt-6 justify-between">
          {!isGenerated ? <Button onClick={handleGenerate} disabled={isGenerating}>
