...
export default function TabsWithIcons({
  ...
  return <div className="w-full flex justify-center mt-4">
      ...
      <div className="w-full max-w-6xl bg-white p-2 rounded-b-lg shadow-md flex items-center justify-center space-x-4">
        {tabs.map(tab => <div key={tab.id} className="relative flex items-center justify-center">
            {}
            {tab.id === "profile" && isUserProfileIncomplete && <TooltipProvider>
                <Tooltip>
            ...
            {}
            <button onClick={() => setActiveTab(tab.id)} className={`relative flex flex-col lg:flex-row items-center gap-2 px-6 py-3 rounded-md text-center font-medium transition-all duration-300 ${activeTab === tab.id ? "bg-green-600 text-white border border-green-500" : "text-gray-600 hover:bg-gray-200"}`}>
  ...
  {activeTab === tab.id && <motion.div layoutId="tab-underline" className="absolute bottom-0 left-0 right-0 h-[2px] bg-green-700 rounded" />}
        </button>
