...
import { Home, Briefcase, User, Phone } from "lucide-react"; // Modern icons
...
// NavItem component for the bottom nav with animation
...
const MobileMenu = () => {
  ...
  userRole === "admin"
   ...
   // ✅ Handle ReportGov Redirect
   const handleReportGovClick = () => {
    if (!user) {
      openSignIn(); // Show Sign-In Modal if not logged in
    } else if (userRole === "admin" ) {
      window.location.href = "/admin"; // Redirect Admin & MDA to /admin
    }  else if (userRole === "mda" ) {
      window.location.href = "/mda"; // Redirect Admin & MDA to /admin
     } else {
      window.location.href = "/user"; // Redirect Regular Users to /user
    }
  };
  ...
  const handleSignOut = async () => {
      ...
      try {
        await signOut();
        router.push("/"); 
        router.refresh(); // Immediately reloads state, helps clear client-side cache.
      } catch (error) {
  ...
  const handleNavClick = (id: string, fromMoreMenu: boolean = false) => {
    if (id !== "notifications") {
      if (fromMoreMenu) {
        setActiveItem(""); // ✅ No active tab in bottom bar if from More
      } else {
        setActiveItem(id); // ✅ Home & Portal remain active when clicked
      }
    }
    setMenuOpen(false); // ✅ Hide drawer when clicking a More tab
  };
  ...
  return (
   ...
   <div className="fixed bottom-0 left-0 w-full bg-black/90 backdrop-blur-lg shadow-lg rounded-t-2xl px-4 py-3 flex justify-around items-center text-gray-400 border-t border-gray-700 z-50">
        ...
        <NavItem
          ...
          onClick={() => setActiveItem("portal")}
        />

        {/* Show Notification Bell if User is ged in, Otherwise Show Contact */}
        {user ? (
          <NavItem
            id="notifications"
            href="#"
        ...
        {/* More Button */}
        <button
          className="relative flex flex-col items-center text-sm text-gray-400"
      ...
      {menuOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end">
          <div className="w-80 bg-white h-full shadow-lg p-6 overflow-y-auto relative rounded-l-2xl mobile-menu">
            ...
            {/* Profile Section */}
            {user ? (
              <div className="flex flex-col items-center gap-4 mt-10 mb-6 p-4 bg-gradient-to-r from-green-500 to-green-700 text-white rounded-lg shadow-md">
          ...
          <div className="py-1">
            ...
            <nav className="mt-2">
              <ul className="space-y-4 text-lg font-medium text-gray-900">
                ...
                {/* About Us Dropdown with Mission & Vision Card */}
                <li>
                  ...
                  {dropdownOpen["about"] && (
                    <div className="ml-4 mt-2 space-y-2 bg-gray-50 p-3 rounded-lg">
                        <ul className="ml-4 mt-2 space-y-2 bg-gray-50 p-3 rounded-lg"> {/* ✅ Now inside <ul> */}
    ...
    <li>
      <Link href="/reforms" className="text-gray-600 hover:text-green-700">
        Reforms
                ...
                {/* Resources Dropdown (Matching Desktop) */}
                <li>
...
const NavItem: React.FC<NavItemProps> = ({ id, href, label, Icon, activeItem, onClick, isNotification, pathname }) => {
  const isActive = pathname === href && !isNotification; // ✅ Compare pathname with href

  return (
    <Link href={href || "#"} onClick={onClick} className="relative flex flex-col items-center text-sm">
      <div className="relative flex items-center justify-center">
        <AnimatePresence>
          {isActive && (
            <motion.div
              initial={{ scale: 0.5, y: 15, opacity: 0 }}
              animate={{ scale: 1.2, y: -20, opacity: 1 }} // ✅ Moves background up more
              exit={{ scale: 0.5, y: 15, opacity: 0 }}
        ...
        <motion.div
          animate={{ y: isActive ? -30 : 0 }} // ✅ Moves icon up when active
