...
import Stepper, { Step } from "@/components/Stepper"; // Import Stepper
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
import { mdasList } from "@/components/mdaList"; // ✅ Import MDA list from component
...
export default function UserTicketForm({ guestMode = false }: { guestMode?: boolean }) {
  ...
  const [ticketNumber, setTicketNumber] = useState<string | null>(null); // Store ticket number
...
const userData = !guestMode ? useQuery(api.users.getUserDetails) : null;
  const getUsersWithRole = useQuery(api.users.getUsersWithRole, { role: "mda" }) || []; // ✅ Fetch MDA users
  const [verificationCodeSent, setVerificationCodeSent] = useState(false); // ✅
  const [phoneError, setPhoneError] = useState("");
  const codeInputsRef = useRef<(HTMLInputElement | null)[]>([null, null, null, null, null, null]);
  const [codeError, setCodeError] = useState("");
const router = useRouter();
  const existingMDAs = useQuery(api.users.getMDAs) || []; // ✅ Fetch MDAs from Convex

  const sendVerificationCode = useAction(api.sendEmail.sendVerificationCode); // ✅ Correct
  const verifyEmailCode = useAction(api.sendEmail.verifyEmailCode); // ✅ fix this path
  ...
  const [form, setForm] = useState({
    ...
    businessName: guestMode ? "" : userData?.businessName ?? "", // ✅ Add this
  ...
  const handleVerifyCode = async () => {
    try {
      ...
      if (result.verified) {
        ...
        setCodeError(""); // ✅ Clear error on success
        toast({ title: "Email verified successfully." });
      } else {
        setCodeError("The code you entered is incorrect."); // ✅ Show error
      }
    } catch (error) {
      setCodeError("Verification failed. Please try again."); // ✅ Catch block
      toast({
        title: "Error",
...
function isStepValid(step: number): boolean {
  switch (step) {
    case 1:
      ...
      // Guest users must verify email before proceeding
      if (guestMode && !form.emailVerified) return false;

      return baseValid;
      case 2:
        // Validate that Assigned MDA is selected and Incident Date is set
        return (
          form.assignedMDA.trim() !== "" &&
          form.incidentDate.trim() !== "" &&
          !checkFutureDate(form.incidentDate) // Make sure the incident date is not in the future
        );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // ✅ Step 1: Create the ticket & get `ticketNumber`
      const { ticketId, ticketNumber } = await createTicket({
        ...
        email: form.email,
         state: form.state,  // ✅ Ensure this is sent
  // address: form.address, // ✅ Ensure this is sent
  businessName: form.businessName, // ✅ Add this
      ...
      // ✅ Step 2: Send Styled Email to the User
      ...
      // ✅ Step 3: Send Styled Email to All Admins
...
`;
  ...
  function handleCloseDialog() {
    ...
    setForm({
      ...
      // address: guestMode ? "" : userData?.address || "",
      assignedMDA: "",
      incidentDate: "",
      supportingDocuments: [],
      businessName: guestMode ? "" : userData?.businessName ?? "", // ✅ Add this
 ...
 return (
  ...
  {/* Stepper Component */}
  <Stepper
  key={modalOpen ? "open" : "closed"} // Ensures Stepper resets when the dialog is closed
  initialStep={1}
  onStepChange={(step) => {
    // Prevent moving past Step 1 if not verified
  ...
  isNextDisabled={!isStepValid(currentStep)} // Pass disabled state based on validation
...
<div className="flex flex-col space-y-1">
  ...
  open={modalOpen} 
  onOpenChange={(open) => {
    if (!open) handleCloseDialog(); // ✅ Reset when clicking outside or closing dialog
  }}
>
  <DialogContent>
    <DialogHeader>
