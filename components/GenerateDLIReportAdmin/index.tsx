...
export function GenerateReportModal({
  ...
  const handleGenerate = async () => {
    ...
    cursorY += 80;
    const selectedRecords = selectedDLIs.includes("ALL") ? allDLIs.filter(d => d.userState === selectedState) : allDLIs.filter(d => d.userState === selectedState && selectedDLIs.includes(d.dliTitle));
    selectedRecords.forEach(dli => {
      ...
      if (dli.status === "completed") statusColor = "#28a745";else if (dli.status === "in_progress") statusColor = "#ffc107";
    ...
    doc.text(`Date: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`, marginX + 400, footerY);
      ...
      <DialogContent className="max-w-2xl">
        ...
        {step === 2 && <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-64 overflow-y-auto">
              ...
              {dliTitles.map(title => <Card key={title} onClick={() => setSelectedDLIs(prev => prev.includes(title) ? prev.filter(t => t !== title) : [...prev.filter(t => t !== "ALL"), title])} className={`p-4 cursor-pointer border ${selectedDLIs.includes(title) ? "border-blue-500" : "hover:border-gray-400"}`}>
                  {title}
                </Card>)}
            </div>
            <DialogFooter>
