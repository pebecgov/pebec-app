...
export const ReportPreview = React.forwardRef<HTMLDivElement, Props>(({
  ...
  return <div ref={ref} className="bg-white text-black px-10 py-8" style={{
    width: "794px",
    height: "1123px",
    fontFamily: "sans-serif"
        ...
        <table className="w-full text-sm border-collapse">
          ...
          return <tr key={idx}>
                  ...
                  <td className={`border p-2 font-medium ${dli.status === "completed" ? "text-green-600" : "text-yellow-600"}`}>
                    {dli.status === "completed" ? "Completed" : "In Progress"}
                  </td>
                  <td className={`border p-2 ${percentage === 100 ? "text-green-700" : "text-yellow-700"}`}>
                    {percentage}%
                  </td>
                </tr>;
        })}
