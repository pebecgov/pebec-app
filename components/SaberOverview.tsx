...
import { FaLandmark, FaNetworkWired, FaHandshake, FaBusinessTime, FaRegListAlt, FaBalanceScale, FaTools, FaBookOpen, FaChartBar } from "react-icons/fa";
import { useQuery } from "convex/react";
import { api } from "@/convex/_generated/api";
const dliCards = [{
  id: 1,
  title: "DLI 1: Land Administration",
  summary: "Improve land transparency through publication of requirements, digitization of land titles, and adoption of FRILIA principles.",
  link: "/dli/1",
  icon: <FaLandmark className="text-blue-600 w-6 h-6" />
}, {
  id: 2,
  title: "DLI 2: Fiber Optic Infrastructure",
  summary: "Facilitate private investment by capping RoW charges, streamlining processes, and expanding fiber deployment.",
  link: "/dli/2",
  icon: <FaNetworkWired className="text-purple-600 w-6 h-6" />
}, {
  id: 3,
  title: "DLI 3: PPP & Investment Promotion",
  summary: "Strengthen legal frameworks and publish transparent pipelines for Public-Private Partnerships.",
  link: "/dli/3",
  icon: <FaHandshake className="text-green-600 w-6 h-6" />
}, {
  id: 4,
  title: "DLI 4: Business Enabling Environment",
  summary: "Reduce regulatory burden and enhance service delivery for businesses through reform tracking.",
  link: "/dli/4",
  icon: <FaBusinessTime className="text-yellow-600 w-6 h-6" />
}, {
  id: 5,
  title: "DLI 5: Regulatory Reform Action Plan",
  summary: "Implement and monitor state-specific Business Enabling Reform Action Plans (BERAPs).",
  link: "/dli/5",
  icon: <FaRegListAlt className="text-indigo-600 w-6 h-6" />
}, {
  id: 6,
  title: "DLI 6: Transparency in Tax Processes",
  summary: "Publish tax procedures and promote voluntary tax compliance via improved systems.",
  link: "/dli/6",
  icon: <FaBalanceScale className="text-rose-600 w-6 h-6" />
}, {
  id: 7,
  title: "DLI 7: Service Efficiency",
  summary: "Digitize and streamline business registration and licensing services.",
  link: "/dli/7",
  icon: <FaTools className="text-gray-700 w-6 h-6" />
}, {
  id: 8,
  title: "DLI 8: Improved Public Access",
  summary: "Ensure the public can easily access business-related regulations and policies.",
  link: "/dli/8",
...
export default function SaberOverviewPage() {
  ...
  const materialsPath = role === "admin" ? "/saber/materials" : `/${role ?? "staff"}/materials`;
  return <div className="bg-[#f9fafb] min-h-screen px-6 py-10 max-w-7xl mx-auto font-sans text-gray-800">
      {}
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {dliCards.map(dli => <div key={dli.id} className="bg-white p-6 rounded-xl shadow hover:shadow-md transition border flex flex-col">
              ...
              <div className="mt-4">
                <Link href={dli.link} className="inline-flex items-center text-blue-600 text-sm hover:underline">
      ...
      <section className="bg-white p-6 rounded-xl shadow mb-16 text-center">
        ...
        <Link href={materialsPath} className="inline-block bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition">
          📁 Go to Materials
        </Link>
      </section>
    </div>;
