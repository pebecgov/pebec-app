...
export const submitLetter = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Notification to Sender (No email)
    await ctx.db.insert("notifications", {
      userId: user._id,
...
// ✅ Get Letters Sent by a Specific User
...
export const getAllSubmittedLetters = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Fetch all users for advanced filters
    ...
    if (args.mdaName || args.staffStream || args.state) {
      ...
      if (userIds.length > 0) {
        ...
        return []; // No matching users = no letters
      }
    }
...
export const updateLetterStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // ✅ Allow both the receiver and the assigned auditor to update
    if (
      !letter ||
