...
export const deleteFromClerk = internalMutation({
  ...
  async handler(ctx, { clerkUserId }) {
    ...
    if (user !== null) {
      // ✅ Remove user from MDA if present
      if (user.mdaId) {
...
async function userByClerkUserId(ctx: QueryCtx, clerkUserId: string | null | undefined) {
  if (!clerkUserId) return null; // Ensure we don't pass undefined/null
  return await ctx.db
    .query('users')
...
export const getUserByClerkId = query({
  args: { clerkUserId: v.optional(v.string()) }, // Allow undefined
  handler: async (ctx, { clerkUserId }) => {
    if (!clerkUserId) return null; // Handle missing values
    return await ctx.db
      .query("users")
      .withIndex("byClerkUserId", (q) => q.eq("clerkUserId", clerkUserId))
      .unique();
...
// ✅ Update user role (for admin assignment)
export const setUserRole = mutation({
  ...
  handler: async (ctx, { userId, role }) => {
    ...
    // ✅ Always update the role immediately
    await ctx.db.patch(userId, { role });

    // ✅ Then clean up if leaving MDA
...
export const getUserRole = query({
  ...
  handler: async (ctx, { clerkUserId }) => {
    ...
    return user?.role ?? "user"; // ✅ Always return "user" if no role is found
  },
});
...
export const getUserRoles = query({
  ...
  handler: async (ctx, { userId }) => {
    const user = await ctx.db
      .query("users")
      .withIndex("byClerkUserId", (q) => q.eq("clerkUserId", userId)) // ✅ Use the correct field name
      .first();
    
    return user?.role || "user"; // Default to "user" if no role is found
  },
});
...
export const getUserById = query({
  ...
  handler: async (ctx, { clerkUserId }) => {
    ...
    return user; // Return the entire user object, not just the userId.
  },
});
...
export const getCurrentUsers = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      phoneNumber: user.phoneNumber,  // ✅ And this
...
export const getAdmins = query({
  ...
  handler: async (ctx) => {
    return await ctx.db
      .query("users")
      .withIndex("byRole", (q) => q.eq("role", "admin")) // ✅ Fetch only admins
      .collect();
  },
});
...
export const updateUserProfile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(user._id, {
      ...
      jobTitle: args.jobTitle, // ✅ Save it
...
export const getUserDetail = query({
  ...
  handler: async (ctx) => {
    const user = await getCurrentUserOrThrow(ctx);
    return user; // ✅ Full user object including _id for comparisons
  },
});
...
export const getUserDetails = query({
  ...
  handler: async (ctx) => {
    ...
    // ✅ Check if user is authenticated, return null if not
    if (!identity) return null;
    ...
    // ✅ If no user details found, return null gracefully
...
export const assignUserToMDA = mutation({
  ...
  handler: async (ctx, { clerkUserId, mdaName, description, phoneNumber }) => {
    ...
    // 2️⃣ Remove user from previous MDA (if any)
    if (user.mdaId) {
    ...
    // 3️⃣ Find or create MDA by name
    ...
    // 4️⃣ Update user with MDA reference
    await ctx.db.patch(user._id, {
      mdaId: mda._id,
      mdaName: mda.name,
...
// export const updateUserRoleInClerk = mutation({
//   args: { clerkUserId: v.string(), role: v.union(v.literal("user"), v.literal("admin"), v.literal("mda")) },
//   handler: async (ctx, { clerkUserId, role }) => {
//     try {
//       const client = clerkClient(); // Call it before using users
//       await (await client).users.updateUserMetadata(clerkUserId, {
//         publicMetadata: { role }, // ✅ Updates the user's role metadata in Clerk
//       });

//       return { success: true, message: "User role updated successfully" };
//     } catch (error) {
//       throw new Error(`Failed to update user role in Clerk: ${error.message}`);
//     }
//   },
// });
...
export const updateUserInConvex = mutation({
  ...
  handler: async (ctx, { clerkUserId, role, mdaId, mdaName, phoneNumber }) => {
    ...
    // If role is MDA, ensure MDA details are stored
    if (role === "mda" && mdaId) {
...
export const updateUserRoleInConvex = mutation({
  args: {
    ...
    state: v.optional(v.string()),
    permissions: v.optional(v.array(v.string())), // ✅ <-- ADD THIS
  ...
  handler: async (ctx, { clerkUserId, role, staffStream, state, permissions }) => {
    ...
    // 🧹 Automatically clear MDA if role changes away from "mda"
    if (user.role === "mda" && role !== "mda" && user.mdaId) {
      // ✅ Pass old mdaId directly
      await ctx.scheduler.runAfter(0, api.users.removeUserFromMDA, {
        clerkUserId,
      });
    ...
    // 🎯 Add permissions if role is staff
    if (role === "staff" && staffStream) {
      ...
      patchData.permissions = permissions ?? undefined; // ✅ Handle admin case
      patchData.staffStream = undefined;
    }

    // 🎯 Add state for state-governor/reform-champion
    if (
      [ "state_governor", "saber_agent", "deputies", "magistrates"].includes(role)
    ) {
      patchData.state = state?.trim() || ""; // Save state or empty string
    } else {
      patchData.state = undefined; // Remove state if role doesn't need it
    }
...
export const deleteMDA = mutation({
  ...
  handler: async (ctx, { mdaId }) => {
    ...
    // Remove MDA details from users first
...
export const getGrowthStats = query(async ({ db }) => {
  const now = new Date();
  const currentMonthStart = new Date(now.getFullYear(), now.getMonth(), 1).getTime(); // ✅ Start of this month
  const lastMonthStart = new Date(now.getFullYear(), now.getMonth() - 1, 1).getTime(); // ✅ Start of last month
  const lastMonthEnd = currentMonthStart - 1; // ✅ End of last month
  ...
  // ✅ Get Users Count (This Month vs Last Month)
  ...
  // ✅ Get Posts Count (This Month vs Last Month)
  ...
  // ✅ Get MDAs Count (This Month vs Last Month)
