...
export const unsubscribeFromNewsletter = mutation({
  ...
  handler: async (ctx, {
    ...
    const subscriber = await ctx.db.query("newsletter_subscribers").withIndex("by_email", q => q.eq("email", email)).first();
...
export const getNewsletters = query({
  ...
  handler: async (ctx, {
    ...
    if (subjectFilter) {
      filtered = filtered.filter(n => n.subject.toLowerCase().includes(subjectFilter.toLowerCase()));
    }
    if (fromDate) {
...
export const getSubscribers = query({
  ...
  handler: async (ctx, {
    ...
    if (emailSearch) {
      results = results.filter(s => s.email.toLowerCase().includes(emailSearch.toLowerCase()));
    }
    const pageSize = 20;
    const start = page * pageSize;
    return {
...
export const batchAddSubscribers = mutation({
  ...
  handler: async (ctx, {
    ...
    const existing = await ctx.db.query("newsletter_subscribers").collect();
