...
export const sendEmail = action({
  ...
  handler: async (_, { to, subject, html }) => {
    const resendApiKey = process.env.RESEND_API_KEY; // ✅ Ensure API key is loaded

    if (!resendApiKey) {
      throw new Error("Missing RESEND_API_KEY in Convex environment.");
    }

    const resend = new Resend(resendApiKey); // ✅ Uses environment variable

    try {
      await resend.emails.send({
        from: "support@pebecgov.com", // ✅ Update with your verified sender
        to,
        subject,
        html,
      });
...
// ✅ New action to send a verification code
export const sendVerificationCode = action({
  args: { email: v.string() },
  handler: async (ctx, args) => {
    const code = Math.floor(100000 + Math.random() * 900000).toString();

    // ⛳ Save the code to the DB via mutation
...
// ✅ DB mutation to store the code (called from the action)
