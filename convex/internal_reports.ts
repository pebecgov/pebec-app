...
// ✅ Mutation to Create a Report Template
...
export const submitReport = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const mdaName = user?.mdaName ?? undefined; // ✅ Safely extract MDA name

    await ctx.db.insert("submitted_reports", {
      ...args,
      mdaName, // ✅ Store it in the report
...
export const getReportTemplates = query({
  ...
  handler: async (ctx, { role }) => {
    ...
    return await ctx.db.query("report_templates").collect(); // ✅ Return all if no role is provided
  },
});
...
// ✅ Query to Get Submitted Reports for a User
      ...
      reports.map(async (report) => {
        ...
        return {
          ...
          userName: user
            ...
            : "Unknown",
            mdaName: user?.mdaName ?? report.mdaName ?? "Unknown", // ✅ fallback to report
          };
      })
    );
...
// ✅ Submit Report to `submitted_reports`
export const submitInternalReport = mutation({
  ...
  handler: async (ctx, args) => {
    const user = await ctx.db.get(args.submittedBy);
    const mdaName = user?.mdaName ?? undefined; // 👈 fixed here
...
export const getAvailableReportsforAdmin = query({
  ...
  handler: async (ctx, { role }) => {
    console.log("Fetching reports for role:", role); // ✅ Debugging

    if (!role || role === "all") {
      return await ctx.db.query("report_templates").collect(); // ✅ Fetch all templates if "all"
    }
    ...
    return await ctx.db
      ...
      .filter(q => 
        q.or(
          q.eq("role", role), 
          q.eq("role", "all") // ✅ Allow both specific role & "all"
        )
      )
      .collect();
  },
...
// ✅ Query to Fetch Submitted Reports Within a Date Range
...
// ✅ Get all submitted reports where the user has role: "deputies"
    ...
    return await Promise.all(
      filtered.map(async (report) => {
        ...
        return {
          ...
          state: user?.state ?? "Unknown", // ✅ include state here
        };
      })
    );
  },
