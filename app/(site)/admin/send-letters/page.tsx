...
export default function ViewLettersPage() {
  ...
  const userMap = Object.fromEntries(allUsers.map(user => [user._id, `${user.firstName || ""} ${user.lastName || ""} (${user.role || "N/A"}${user.jobTitle ? `, ${user.jobTitle}` : ""})`.trim()]));
  ...
  const filtered = allLetters.filter(l => {
    ...
    const matchStatus = filters.status ? (l.status ?? "sent") === filters.status : true;
    const matchRole = filters.role ? l.sentTo !== undefined && roleMap[l.sentTo] === filters.role : true;
  ...
  return <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <Input placeholder="Search by letter name" value={filters.search} onChange={e => setFilters({
        ...filters,
        ...
        <Input type="date" value={filters.dateFrom} min="2024-10-22" onChange={e => {
      ...
      <Input type="date" value={filters.dateTo} min={filters.dateFrom || "2024-10-22"} onChange={e => {
        const value = e.target.value;
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow">
        <Table className="min-w-full text-sm">
          ...
          <TableBody>
            {paginated.length > 0 ? paginated.map(letter => <TableRow key={letter._id}>
