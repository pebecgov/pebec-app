...
const roles = ["admin", "user", "mda", "staff", "reform_champion", "federal", "deputies", "magistrates", "state_governor", "president", "vice_president"] as const;
...
export default function SaberMaterialsPage() {
  ...
  const filteredMaterials = selectedRole === "all" ? allMaterials : allMaterials?.filter(m => m.roles.includes(selectedRole));
  ...
  return <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex justify-between items-center mb-6">
        ...
        {userRole === "admin" && <Button onClick={() => setShowAddModal(true)} className="bg-blue-600 text-white">
            + Add Material
          </Button>}
      </div>
      ...
      {filteredMaterials ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredMaterials.map(mat => <div key={mat._id} className="card w-full min-h-[280px] bg-[#07182E] rounded-2xl overflow-hidden relative transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,183,255,0.5)] flex flex-col">
                ...
                <div>
                  <div className="flex items-start mb-4 gap-3">
                    ...
                    <div className="flex-1">
                      <h2 className="text-lg font-bold text-white/90 break-words leading-snug line-clamp-2" title={mat.title}>
                        {mat.title}
                      </h2>
              ...
              if (url) window.open(url, "_blank");else toast.error("Could not retrieve download link");
                  ...
                  {userRole === "admin" && <>
                      <Button size="sm" variant="ghost" className="text-white" onClick={() => setEditingMaterial(mat)}>
                        <Pencil className="w-4 h-4" />
                      </Button>
                      <Button size="sm" variant="ghost" className="text-red-500" onClick={() => handleDelete(mat._id)}>
        ...
        </div> : <p className="text-sm text-muted-foreground mt-4">Loading materials...</p>}
