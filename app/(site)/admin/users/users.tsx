...
import { mdasList } from "@/components/mdaList"; // ✅ Import MDA List
...
export default function Admin() {
  ...
  const [selectedUser, setSelectedUser] = useState<any>(null);  // ✅ Now can hold full user object
  ...
  useEffect(() => {
    if (selectedUser) {
      ...
      if (user) {
        ...
        setSelectedStream(user.staffStream ?? ""); // ✅ prefill stream if staff
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setRole(formData); // Clerk side
      ...
      // Remove user from MDA if role is no longer MDA-type and user had an MDA before
      if (!["mda", "reform_champion"].includes(selectedRole) && wasInMDA) {
        await removeUserFromMDA({ clerkUserId: selectedUser });
      }
  
      // Reassign if role is now MDA-type
  ...
  .filter((user) => user.clerkUserId && !user.clerkUserId.startsWith("guest_")) // ✅ Filter out guest users
  .filter((user) => {
...
>
  ...
  onValueChange={(val) => {
    ...
    setHasManuallySelectedRole(true); // ✅ prevent auto-reset
  }}
  value={selectedRole}
>
