...
export default function WorldBankDLIProgressPage() {
  ...
  const dliTemplate = useQuery(api.dli.getDliTemplate, 
    dliProgress?.dliTemplateId ? {
      id: dliProgress.dliTemplateId
    } : "skip"
  );

  // Use the safer function that returns null instead of throwing
  const user = useQuery(api.users.getUserByIdSafe, 
    dliProgress?.userId ? {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6">
      {/* Back Button */}
      <Button 
        onClick={() => router.push("/world_bank")} 
      ...
      <div className="mb-8">
        <Progress 
          value={(dliProgress.completedSteps / dliProgress.totalSteps) * 100} 
          color="green" 
        />
        ...
        {dliProgress.steps.map((step, index) => {
          ...
          return (
            <div key={index} className="relative">
              ...
              <Card className={`relative z-10 p-4 shadow-sm rounded-xl transition-all
                ${isCompleted 
