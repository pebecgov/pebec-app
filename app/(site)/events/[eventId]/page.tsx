...
import html2canvas from "html2canvas"; // ✅ Replacing dom-to-image
...
const [vipCode, setVipCode] = useState("");
  ...
  const event = useQuery(api.events.getEventById, isClient ? { eventId: eventId as Id<"events"> } : "skip");
  ...
  useEffect(() => {
    setIsClient(true); // ✅ Ensures `window` and `document` exist
  }, []);
  ...
  // ✅ Ensure queries only run on client-side
  ...
  const generateStyledPdfBlobFromTicket = async (ticketObject) => {
    ...
    const pdf = new jsPDF({
      ...
      unit: "mm",
      format: [210, 80], // ✅ Rounded corners applied
    });
  
    // 🎨 **Rounded Background for the Entire Ticket**
    pdf.setFillColor(255, 255, 255); // White Background
    pdf.roundedRect(5, 5, 200, 70, 15, 15, "F"); // ✅ Rounded edges applied to full ticket
  
    // 🎨 **Left Section: QR Code Background (Light Grey)**
    ...
    // 🎨 **Right Section: Dark Green with White Text (Expands if Needed)**
    pdf.setFillColor(0, 85, 50);
    pdf.roundedRect(145, 10, 55, 60, 10, 10, "F");
  
    // 🔳 **Add Perforated Line (Full Ticket Height)**
    pdf.setDrawColor(150);
    pdf.setLineDashPattern([2, 2], 0);
    pdf.line(67, 10, 67, 70);
  
    // 🏛 **Add Logo Above Event Name (Fixed Aspect Ratio & Spacing)**
    const logoPath = "/images/logo/logo_pebec1.PNG";
    pdf.addImage(logoPath, "PNG", 88, 12, 30, 12); // ✅ Proper spacing added
  
    // 🎟 **Middle Section: Event Name (Centered)**
    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.setFont("helvetica", "bold");
    const eventTitleLines = pdf.splitTextToSize(ticketObject.event.title.toUpperCase(), 65);
    pdf.text(eventTitleLines, 105, 35, { align: "center" }); // ✅ Fully Centered
  
    // 🔢 **Ticket Number (Bottom Center)**
    pdf.setFontSize(10);
    pdf.setTextColor(100, 100, 100);
    const ticketNumberLines = pdf.splitTextToSize(`Ticket #: ${ticketObject.ticketNumber}`, 65);
    pdf.text(ticketNumberLines, 105, 65, { align: "center" }); // ✅ Moved lower
    pdf.setFont("helvetica", "bold");
pdf.setFontSize(10);
...
pdf.text(`Event Type: ${formattedType}`, 105, 60, { align: "center" });
    ...
    // 📅 **"Generated on" Date Format (Moves Lower Dynamically)**
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(8);
    ...
    pdf.setFontSize(12);
    pdf.setTextColor(30, 30, 30);
    pdf.text(eventDate.toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }), 15, 63);
  
    // 📍 **Right Section: Location (Wrapped & Responsive)**
    pdf.setFontSize(12);
...
if (!currentUser) {
  // Check if email already used for this event
  ...
  if (existing) {
    ...
    try {
      ...
      // ✅ Final RSVP submission with all correct data
...
const generatePdfFromTicket = async () => {
  ...
  // ✅ Ensure ticket is visible for capture
  ticketElement.style.display = "block";
  ticketElement.style.position = "relative";
  ticketElement.style.width = "fit-content"; // Prevents stretching
  ticketElement.style.height = "auto";
  ticketElement.style.minHeight = "fit-content"; // Ensures full content
  ...
  try {
    ...
    // ✅ Hide ticket after capture
    ticketElement.style.display = "none";
    setTicketData(null); // ✅ Reset ticket state so it disappears
  }
};
