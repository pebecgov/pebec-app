...
export default function StaffLettersPage() {
  ...
  const filteredLetters = letters.filter(l => l.assignedStream === staffStream).filter(l => {
    ...
    const matchesDate = (!filter.from || filter.from <= recordDateStr) && (!filter.to || recordDateStr <= filter.to);
  ...
  return <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end gap-4 mb-6 flex-wrap">
        ...
        <div className="flex gap-2">
          <Input type="date" value={filter.from} onChange={e => setFilter({
          ...filter,
          from: e.target.value
        })} />
          <Input type="date" value={filter.to} onChange={e => setFilter({
          ...filter,
          to: e.target.value
        })} />
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="ghost" size="sm" className="text-red-500" onClick={() => setFilter({
          email: "",
          from: "",
          to: ""
        })}>
  ...
  <tbody>
    {paginatedLetters.map(letter => {
            const statusFlow: Record<"pending" | "acknowledged" | "in_progress" | "resolved", "acknowledged" | "in_progress" | "resolved" | null> = {
              pending: "acknowledged",
              acknowledged: "in_progress",
              in_progress: "resolved",
              resolved: null
            ...
            return <tr key={letter._id} className="border-t">
          ...
          <td className="p-3">
            <span className={`capitalize px-2 py-1 rounded text-xs font-medium ${letter.status === "resolved" ? "bg-green-100 text-green-700" : letter.status === "in_progress" ? "bg-blue-100 text-blue-700" : letter.status === "acknowledged" ? "bg-purple-100 text-purple-700" : "bg-orange-100 text-orange-700"}`}>
              {letter.status || "pending"}
            </span>
          </td>

          <td className="p-3">
            {nextStatus ? <Button variant="outline" size="sm" onClick={() => handleStatusChange(letter._id, nextStatus)}>
              ...
              </Button> : <span className="text-xs text-gray-400">No further action</span>}
          </td>
        </tr>;
          })}
  </tbody>
