  ...
  export default function ViewLettersPage() {
    ...
    const allUsers = useQuery(api.users.getUsers) || []; // ✅ Must return _id, firstName, lastName, role
    ...
    useEffect(() => {
      const fetchFileUrls = async () => {
        const urls: { [key: string]: { url: string; fileName: string } } = {}; // ✅ Correct type
          ...
          if (letter.letterUploadId && !fileUrls[letter._id]) {
            try {
              ...
              if (response) {
                urls[letter._id] = {
                  url: response.url,
                  fileName: response.fileName || "Letter.pdf", // ✅ Safe fallback
                };
              }
            } catch (error) {
