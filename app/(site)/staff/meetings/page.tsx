...
export default function StaffAvailabilityPage() {
  ...
  const handleCreate6Slots = async () => {
    ...
    const base = setHours(setMinutes(new Date(startDateTime), 0), 10); // 10:00 AM
    let created = 0;
  
    for (let i = 0; i < 6; i++) {
      const slotStart = addMinutes(base, i * 60);
      if (isBefore(slotStart, now)) continue; // skip past times
  ...
  const visibleSlots = useMemo(() => {
    ...
    if (activeTab === "upcoming") {
      return availability.filter(slot => {
        ...
        return (
          slot.bookedBy &&
          !(slot as any)?.cancelled && // exclude cancelled
          isAfter(start, now) && // exclude past
