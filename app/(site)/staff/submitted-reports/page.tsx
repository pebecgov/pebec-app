...
export default function SubmittedReportsPage() {
  ...
  const deleteReport = useMutation(api.internal_reports.deleteSubmittedReport); // ✅ Added Mutation

  // ✅ Fetch storage URLs for uploaded files
 ...
 const paginatedReports = filteredReports.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const exportToPDF = (report) => {
    ...
    autoTable(doc, {
      startY: 35,
      head: [template.headers.map((h) => h.name)], // Headers from template
      body: report.data,
      theme: "striped",
  ...
  const handleDeleteReport = async (reportId: string) => {
    ...
    try {
      await deleteReport({ id: reportId as Id<"submitted_reports"> }); // ✅ Convert to the expected type
  ...
  onClick={() => {
    ...
    link.download = fileMeta.fileName || "downloaded_file"; // ✅ Use real filename
    link.target = "_blank";
