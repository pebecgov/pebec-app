...
const roles = ["admin", "user", "mda", "staff", "reform_champion", "federal", "deputies", "magistrates", "state_governor", "president", "vice_president", "saber_agent"] as const;
...
export default function SaberMaterialsPage() {
  ...
  const roleMaterials = userRole ? useQuery(api.saber_materials.getSaberMaterialsByRole, {
    role: userRole
  }) : undefined;
  if (userRole === "admin") {
  ...
  return <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      {materials ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {materials.map(mat => <div key={mat._id} className="w-full bg-[#07182E] rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,183,255,0.5)] flex flex-col min-h-[300px]">
