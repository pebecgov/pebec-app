...
export default function MediaPage() {
  ...
  return <div className="max-w-7xl mx-auto mt-20">
      ...
      <div className="flex flex-wrap gap-4 justify-center mt-10 mb-8 px-4">
        <select className="border px-3 py-2 rounded w-48" value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4 pb-10">
        {filtered.slice(0, limit).map(item => <div key={item._id} className="bg-white border rounded-lg shadow hover:shadow-md transition overflow-hidden">
            <div className="relative w-full h-48">
              <Image src={item.coverImageUrl || "/images/media_cover.png"} alt="cover" fill className="object-cover" />
            </div>
            ...
            <div className="p-4 flex flex-col justify-between h-[200px]">
              ...
              <Link href={`/media/${item._id}`} className="mt-3 text-sm font-medium text-blue-600 hover:underline">
                View All
              </Link>
            </div>
          </div>)}
