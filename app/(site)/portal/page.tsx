...
import { Sparkles, Building2, ChevronDown, ShieldCheck, Layers } from "lucide-react";
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (sectorDropdown && !sectorDropdown.contains(event.target as Node) &&
        sectorMenu && !sectorMenu.contains(event.target as Node)) {
        setIsSectorDropdownOpen(false);
      }
      if (subsectorDropdown && !subsectorDropdown.contains(event.target as Node) &&
        subsectorMenu && !subsectorMenu.contains(event.target as Node)) {
          ...
          <div className="flex flex-col-reverse lg:flex-row items-center gap-10">
            <div className="w-full lg:w-1/2 text-center lg:text-left">
              ...
              <p className="mt-4 text-gray-600 text-base md:text-lg max-w-xl mx-auto lg:mx-0">
                Navigate Nigerian government services with ease. Select your business sector and subsector to view your tailored roadmap.
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative flex-1">
              ...
              {isSectorDropdownOpen && (
                <div id="sector-dropdown-menu" className="absolute top-full mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-[400px] overflow-y-auto">
                  {SECTORS.map((sector) => (
                    <button
                      key={sector.name}
                      onClick={(e) => {
                      ...
                      className={`w-full text-left px-4 py-3 text-sm font-medium transition-colors ${selectedSector === sector.name
            ...
            {selectedSector && (
              <div className="relative flex-1">
                ...
                >
                  ...
                  <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform ${isSubsectorDropdownOpen ? "rotate-180" : ""}`} />
                </button>

                {/* Subsector Dropdown Menu */}
                {isSubsectorDropdownOpen && (
                  <div id="subsector-dropdown-menu" className="absolute top-full mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-[400px] overflow-y-auto">
                    {availableSubsectors.map((subsector) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 text-sm font-medium transition-colors ${selectedSubsector === subsector
