...
export default function FrameworksPage() {
  ...
  const paginatedFrameworks = sortedFrameworks.slice(page * ITEMS_PER_PAGE, (page + 1) * ITEMS_PER_PAGE);
  const totalPages = Math.ceil((frameworks?.length || 0) / ITEMS_PER_PAGE);
  return <div className="pb-20 mt-25">
      {}
      {}
  ...
  <div className="max-w-7xl mx-auto px-4 py-16 flex flex-col md:flex-row items-center justify-between gap-10">
    ...
    <div className="w-full md:w-[300px] lg:w-[360px]">
      <img src="/images/frameworks.svg" alt="Frameworks Icon" className="w-full h-auto object-contain" />
    </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedFrameworks.map(item => <Card key={item._id} className="flex flex-col justify-between h-full border shadow-sm hover:shadow-md transition-all">
              ...
              <CardContent className="flex flex-col flex-grow justify-between gap-3 mt-2">
                ...
                <div className="text-xs text-gray-500 mt-2 flex items-center gap-1">
                  📁{" "}
                  {item.fileSize > 0 ? `${(item.fileSize / 1024 / 1024).toFixed(2)} MB` : "< 0.01 MB"}
                </div>

                <a href={frameworkUrls[item._id]} target="_blank" rel="noopener noreferrer" className="mt-auto">
