...
export default function MdaSlots() {
  ...
  const staffAvailableSlots = useQuery(api.meetings.getAvailableSlotsForStaff, selectedStaffUser ? {
  ...
  const staffWorkstreams = useMemo(() => {
    const workstreamSet = new Set(staffUsers.filter(u => u.role === "staff" && u.staffStream).map(u => u.staffStream!));
    return Array.from(workstreamSet);
  }, [staffUsers]);
  const filteredSlots = useMemo(() => {
    if (!selectedStaffUser) return [];
    return allSlots.filter(slot => slot.userId === selectedStaffUser && !slot.deactivated);
  }, [allSlots, selectedStaffUser]);
  const bookedSlots = useMemo(() => {
    return allSlots.filter(slot => slot.bookedBy === convexUser?._id && !slot.deactivated).sort((a, b) => parseISO(`${a.date}T${a.startTime}`).getTime() - parseISO(`${b.date}T${b.startTime}`).getTime());
  }, [allSlots, convexUser]);
  const slotsForDay = useMemo(() => {
    if (!selectedDate) return [];
    return filteredSlots.filter(slot => isSameDay(parseISO(`${slot.date}T${slot.startTime}`), selectedDate));
  ...
  const firstAvailableSlot = useMemo(() => {
    ...
    }).sort((a, b) => parseISO(`${a.date}T${a.startTime}`).getTime() - parseISO(`${b.date}T${b.startTime}`).getTime())[0] || null;
  }, [filteredSlots, selectedStaffUser]);
  useEffect(() => {
    if (activeTab === "upcoming" && bookedSlots.length > 0 && !selectedDate) {
      setSelectedDate(parseISO(`${bookedSlots[0].date}T${bookedSlots[0].startTime}`));
    }
  }, [bookedSlots]);
  ...
  const tileContent = ({
    ...
    const relevantSlots = selectedStaffUser ? filteredSlots.filter(slot => isSameDay(parseISO(`${slot.date}T${slot.startTime}`), date)) : bookedSlots.filter(slot => isSameDay(parseISO(`${slot.date}T${slot.startTime}`), date));
  ...
  <div className="relative">
    <select className="block w-full appearance-none bg-white border border-gray-300 text-gray-900 text-sm rounded-md px-4 py-2 pr-10 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value={selectedWorkstream || ""} onChange={e => {
    ...
    <div className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-gray-500">
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          ...
          {}
          {selectedWorkstream && <div className="mb-3">
              <label className="block text-sm font-medium mb-1">Select Staff:</label>
              <select className="w-full p-2 border rounded" value={selectedStaffUser || ""} onChange={e => setSelectedStaffUser(e.target.value as Id<"users">)}>
  <option value="">-- Select Staff --</option>
  {staffUsers.filter(u => u.role === "staff" && u.staffStream === selectedWorkstream).map(u => <option key={u._id} value={u._id}>
    ...
    <div className="text-sm">
      ...
      <p className="text-xs text-gray-600 mt-1 italic">
        ...
        }} value={selectedDate} tileContent={tileContent} />
        </div>
        ...
        <div className="flex-1 bg-white p-4 rounded-xl shadow max-h-[600px] overflow-y-auto">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-3">
            <h2 className="text-lg font-semibold">
              {activeTab === "available" ? `Schedule for ${format(selectedDate ?? new Date(), "EEEE, MMM d")}` : showAllMeetings ? "📋 All Upcoming Appointments" : `📋 Appointments for ${format(selectedDate ?? new Date(), "EEEE, MMM d")}`}
            </h2>
          ...
          {visibleSlots.map(slot => {
            ...
            return <div key={slot._id} className="border rounded-lg px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-gray-50">
      <div className="flex flex-col gap-1">
        ...
        {activeTab === "upcoming" && <p className="text-sm text-gray-700">
            ...
            {(() => {
                    ...
                    return staff ? `${staff.firstName} ${staff.lastName}${staff.jobTitle ? ` — ${staff.jobTitle}` : ""}` : "Unknown Staff";
                  })()}
          </p>}
      </div>

      {isBookedByYou ? <Button variant="destructive" size="sm" className="w-full sm:w-auto" onClick={() => handleCancel(slot._id as Id<"availability">)}>
          Cancel
        </Button> : activeTab === "available" ? (() => {
                const slotDate = parseISO(`${slot.date}T${slot.startTime}`);
                const now = new Date();
                const isPast = slotDate < now;
                const isDisabled = !!slot.bookedBy || isPast;
                return <Button size="sm" disabled={isDisabled} onClick={() => handleBookSlot(slot._id as Id<"availability">)} className={isDisabled ? "opacity-50 cursor-not-allowed" : ""}>
              {slot.bookedBy ? "Booked" : isPast ? "Expired" : "Book"}
            </Button>;
            ...
            {visibleSlots.length === 0 && <p className="text-gray-500 text-sm">
                No{" "}
                {activeTab === "available" ? "available slots. Make sure you selected department and staff." : "appointments found."}
              </p>}

            {activeTab === "upcoming" && bookedSlots.length > 0 && <div className="text-right mt-4">
