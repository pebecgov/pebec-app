...
export default function ReportsPage() {
  ...
  const filteredReports = useMemo(() => {
    ...
    let results = allReports.filter(report => report.title.toLowerCase().includes(searchQuery.toLowerCase()));
    if (startDate && endDate) {
      results = results.filter(report => report.publishedAt >= startDate.getTime() && report.publishedAt <= endDate.getTime());
    }
    return results;
  ...
  return <main className="pb-16 mt-25">
        ...
        <div className="text-center max-w-2xl">
          ...
          <div className="mt-6 flex justify-center">
            <div className="relative flex items-center w-full max-w-lg bg-[#73781d] shadow-[2px_2px_20px_rgba(0,0,0,0.08)] rounded-full overflow-hidden transition-all duration-300 hover:w-[350px]">
              ...
              {}
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search reports..." className="w-full bg-transparent outline-none text-white text-lg px-4 placeholder-white" />
            </div>
          </div>
        </div>
        {}
      ...
      <div className="container mx-auto px-6 mt-8 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-100 p-4 rounded-lg shadow">
        <div className="flex items-center gap-2">
          ...
          <DatePicker selected={startDate} onChange={date => setStartDate(date)} placeholderText="Select Start Date" className="p-2 border border-gray-300 rounded-md text-black" />
        </div>
        <div className="flex items-center gap-2">
          <label className="font-medium">To Date:</label>
          <DatePicker selected={endDate} onChange={date => setEndDate(date)} placeholderText="Select End Date" className="p-2 border border-gray-300 rounded-md text-black" />
        </div>
      </div>
      ...
      <section className="container mx-auto px-6 mt-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredReports.map(report => <ReportCard key={report._id} report={report} />)}
        </div>
      </section>
    </main>;
}